...
#
excelpath="Habitat Data/" ## Modify this to reflect the directory with all files to be analyzed
...
# Sheet 1:
  ...
  Nutritional Data required (with units):
    ...
    Column 5: DN (digestible N: g/g DM)
    Column 6: DN.SE (standard deviation)
  ...
  Sheet 2 Name = "metadata"
    ...
    Column 7: DNconst (minimum digestible N (digestible protein/6.25) required by the animal: g/day)
    Column 8: DMconst (maximum dry matter intake constraint: g DM/day)
...
#parameters are:  MEReq = metabolizable energy requirement, DMIconst = the dry matter intake constraint,  and DNconst = the digestible N constraint
...
olddata=data.frame(data$Species,data$Biomass/3,data$DE,data$DN,data$Max)
colnames(olddata)[1]="Species"
colnames(olddata)[2]="Biomass"
colnames(olddata)[3]="DE"
colnames(olddata)[4]="DN"
...
for(i in 1:nrow(data)){
	...
	dn1[i]=data$DN[i]-data$DN.SD[i]
	dn2[i]=data$DN[i]+data$DN.SD[i]
...
Biomass=c(bio1,bio2)
Max=c(max,max)
newdata=data.frame(Species,Biomass,DE,DN,Max)
...
cde=0
cdn=0
for(l in 1:nrow(fdata)){ #this loop calculates the de and dn for each food relative to the constrained requirements
	cde[l]=fdata$DE[l]*fdata$Biomass[l]-fdata$Biomass[l]*de
	cdn[l]=fdata$DN[l]*fdata$Biomass[l]-fdata$Biomass[l]*dn
}
f.con=c(cde,cdn)
f.con=c(f.con,conm)
...
totDN=sum(fdata$DN*bioused)
dnave=totDN/sum(bioused)
totBio=sum(bioused)
MEintake=deave*parms[n,3]
out2=data.frame(fdata,bioused,used,pctbioused)
out3=data.frame(newsite,sum(fdata$Biomass),totBio,deave,MEintake,parms[n,1],dnave,parms[n,2],parms[n,3],totBio/(parms[n,3]/1000))
...
colnames(out3)[7]="Ave DN"
colnames(out3)[8]="DN Const"
...
result=paste0(c("Habitat Output/",site2),collapse="")
saveWorkbook(outname,result,overwrite=TRUE)
} #end habitat loop
#
writeData(outname3,out4,sheet="summary",startRow=1,startCol=1)
saveWorkbook(outname3,"Habitat Output/RunSummary.xlsx",overwrite=TRUE)
